...
graph TB
    ...
    subgraph "业务服务层"
        ...
        XRD_FLOW_MGR[XRD流程管理器<br/>XRDFlowManager]
    end
    ...
    subgraph "设备控制层"
        ...
        XRD_CTRL[XRD控制器<br/>XRDController<br/>TCP Socket]
    end
    ...
    subgraph "通信协议层"
        ...
        TCP_SOCKET_PROTO[TCP Socket<br/>TCP]
    ...
    EXP_API --> MIXER_SVC
    EXP_API --> FLOW_MGR
    EXP_API --> XRD_FLOW_MGR
    EXP_API --> MIXER_CTRL
    EXP_API --> XRD_CTRL
    
    FLOW_API --> FLOW_MGR
    XRD_API --> XRD_FLOW_MGR
    XRD_API --> XRD_CTRL
...
flowchart TD
    ...
    UNLOAD_SHELF --> XRD_PREPARE[XRD设备准备<br/>开高压/设置参数]
    XRD_PREPARE --> XRD_SAMPLE[XRD上样]
    XRD_SAMPLE --> XRD_TEST[XRD测试]
    XRD_TEST --> XRD_WAIT{XRD完成?}
    XRD_WAIT -->|否| XRD_WAIT
    XRD_WAIT -->|是| XRD_DOWN[XRD下样]
    XRD_DOWN --> XRD_DATA[获取XRD数据]
    XRD_DATA --> END([实验完成])
...
sequenceDiagram
    participant Client as 客户端
    participant API as XRD API
    participant XFM as XRDFlowManager
    participant XRD as XRD控制器
    
    rect rgb(86, 87, 88)
        Note over Client,XRD: 单样品测试流程
        Client->>API: POST /api/xrd/single_sample_test
        API->>XFM: single_sample_test(sample_id, params)
        
        XFM->>XRD: get_sample_request()
        XRD-->>XFM: 允许上样
        
        Note over XFM: 等待人工确认
        Client->>API: POST /api/xrd/confirm
        API->>XFM: user_confirm()
        
        XFM->>XRD: send_sample_ready(sample_id, params)
        XRD-->>XFM: 参数发送成功
        
        Note over XRD: 设备自动测试
        
        XFM->>XRD: get_current_acquire_data()
        XRD-->>XFM: 测试数据
        
        Note over XFM: 等待人工确认下样
        Client->>API: POST /api/xrd/confirm
        API->>XFM: user_confirm()
        
        XFM->>XRD: get_sample_down(station)
        XFM->>XRD: send_sample_down_ready()
        XRD-->>XFM: 下样完成
    ...
    rect rgb(143, 142, 141)
        Note over Client,XRD: 多样品测试流程
        Client->>API: POST /api/xrd/multi_sample_test
        API->>XFM: multi_sample_test(samples)
        ...
        loop 每个样品
            ...
            XFM->>XRD: send_sample_ready(sample_info)
            XRD-->>XFM: 参数发送成功
        end
        
        Note over XRD: 设备自动测试所有样品
...
```mermaid
flowchart TD
    START([开始XRD测试]) --> CHECK[检查设备连接]
    CHECK -->|未连接| CONNECT[连接XRD设备]
    CHECK -->|已连接| POWER_ON[开启高压发生器]
    CONNECT --> POWER_ON
    
    POWER_ON --> SET_VOLTAGE[设置电压电流]
    SET_VOLTAGE --> AUTO_MODE[启动自动模式]
    AUTO_MODE --> READY[设备就绪]
    
    READY --> SAMPLE_REQ[检查上样请求]
    SAMPLE_REQ -->|允许| WAIT_SAMPLE[等待人工上样]
    SAMPLE_REQ -->|拒绝| ERROR[错误处理]
    
    WAIT_SAMPLE --> SEND_PARAMS[发送采集参数]
    SEND_PARAMS --> TESTING[设备测试中]
    
    TESTING --> GET_DATA[获取测试数据]
    GET_DATA --> WAIT_DOWN[等待人工下样]
    WAIT_DOWN --> DOWN[下样完成]
    ...
    style WAIT_SAMPLE fill:#FFD700
    style WAIT_DOWN fill:#FFD700
...
graph LR
    ...
    subgraph "通信协议"
        ...
        TCP_SOCKET[TCP Socket<br/>TCP:8009]
...
3. **API路由层**：9个API路由模块，提供RESTful接口（新增XRD API）
4. **业务服务层**：配料服务、流程管理器、XRD流程管理器
5. **设备控制层**：6个设备控制器，统一继承BaseDevice
6. **通信协议层**：5种通信协议（PLC、Modbus、ZMQ Socket、TCP Socket、REST）
7. **硬件设备层**：6种物理设备

### 关键特性

- **统一设备接口**：所有设备继承BaseDevice，提供统一的操作接口
- **流程编排**：FlowManager协调多个设备完成复杂流程，XRDFlowManager管理XRD试验流程
- **人工确认机制**：关键步骤需要人工确认，确保安全
- **状态管理**：完整的设备状态和任务状态管理
- **配置管理**：通过.env文件统一管理配置
- **日志系统**：完整的日志记录和查询功能
- **XRD支持**：支持单样品和多样品（最多30个）测试模式，完整的XRD试验工作流