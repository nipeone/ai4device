...
def get_door_status():
    status_dict = {}
    for i in range(1, 7): status_dict[i] = door_controller.get_door_status(i)
...
def control_door(id: int, action: str):
    ...
    res = door_controller.send_command(id, action)
    res["current_real_status"] = door_controller.get_door_status(id)
    return res